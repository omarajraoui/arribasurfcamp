...
const BookingSummary = ({ dates, participants, total, weekPrice }) => {
  ...
  const totalDisplay = start ? formatPrice(weekPrice * participants / weekPrice * BASE_WEEK_PRICE_EUR * participants, lng) : '---';
  return (
    <div className="space-y-8">
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-6 space-y-4 text-sm">
        ...
        <div className="border-t border-gray-200 pt-4 flex justify-between font-semibold text-brand-ink"><span>{t('bookingSummary.total')}</span><span>{weekPrice && participants ? formatPrice(BASE_WEEK_PRICE_EUR * participants, lng) : '---'}</span></div>
      </div>
